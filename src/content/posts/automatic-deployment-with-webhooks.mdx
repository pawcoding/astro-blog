...
The first step in the deployment process is to generate a new relese.
...
These commit messages are then analyzed by semantic-release inside a GitHub action, everytime I push something to the repository.
...
But as I use this mostly for standalone frontend projcts, I use these major versions to signalize major big features or (partial) rewrites of the project.
...
This makes it easy to see what has beed added, changed or fixed in the new version (again, this is more useful for libraries or products than just websites).
...
5. The programm is restarted with the new version
...
# Restart the service
pm2 restart ecosystem.confitg.js --env production
```
...
No problem, just send the errors directly from your programm to a dedicated topic.
