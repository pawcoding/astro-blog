...
During the build process, Astro reads the content from these documents, parses the frontmatter (in a typesafe way) and transforms the markdown content into HTML.
...
This way you can access the content in a typesafe way in your components.
...
This again makes sure that the logic in our components is typesafe and correct.
...
If you're interested in these details, I recommend checking out the [GitHub repository](https://github.com/pawcoding/astro-loader-pocketbase) for the full souce code.
...
Astro also has a new [API for typesafe environment variables](https://astro.build/blog/astro-5-beta/#stable-astroenv) in version 5, but I still have to figure out if this can be used to store the credentials more securely._
