...
description: "See how to establish an event-based communication between scalable microservices using Redis Streams. We'll cover the basic concepts, operations and some tips for production usage."
...
So we can not delete the consumer during it's own shutdown, but need to do it every night, while checking for any pending messages before deleting it.
...
For more information, check out the awesome [Introduction to Redis Streams](https://redis.io/docs/latest/develop/data-types/streams/) documentation right from the source.
